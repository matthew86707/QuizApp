...
public class ServerMain implements TCPConnectionCallback {
			...
			public void run() {
				while (true) {
					...
					if (currentState == ServerState.AWAITING_ANSWERS && clients > 0) {
						// Hold thread in this state until everyone has
						// answered, then switch the state to AWAITING_SELECTION
						// for the next time around the loop
						if (answered >= clients) {
							try {
								// TODO : Sometimes, becuase this is checkign so
								// often, it will see that all the answers have
								// been in and reset the correct variable
								// before all the places can be calculated,
								// causing the last person in to get 1st place
								// because the var is reset to 0
								Thread.sleep(2500);
							...
							answered = 0;
							answeredCorrect = 0;
							// TODO : Add back in, commented out for testing on
							// windows...
	...
	public static void sendPlayersInfo() {
		...
		DataPackage dp = new DataPackage((Serializable[]) players);
		for (Player p : Player.allPlayers) {
			try {
