...
public class MouseHandler implements MouseListener{
	...
	public void mouseClicked(MouseEvent e) {
	    ...
	    if(!(ClientMain.finalOption.deltaHearts == 0)){
	    ...
	    for(int i = 0; i < Player.allPlayers.size(); i++){
	    	for(int j = 0; j < Player.allPlayers.get(i).hearts.size(); j++){
	    		if(x < (Heart.STATIC_OFFSET_X + Heart.OFFSET_X)){
	    			if(y > (Player.STATIC_OFFSET_Y + (i * Player.OFFSET_Y) - 30) && y < (Player.STATIC_OFFSET_Y  + (i * Player.OFFSET_Y) + Heart.SIZE_Y)){
		    			if(ClientMain.finalOption.deltaHearts < 0){
		    				Player.allPlayers.get(i).hearts.remove(0);
		    				ClientMain.finalOption.deltaHearts++;
		    			}else{
		    				Player.allPlayers.get(i).hearts.add(new Heart());
		    				ClientMain.finalOption.deltaHearts--;
		    			}
	    ...
	    for(int i = 0; i < ClientMain.currentOptions.options.size(); i++){
	    	if(x > (Option.STATIC_OFFSET_X + (i * Option.OFFSET_X)) && x < (Option.STATIC_OFFSET_X + (i * Option.OFFSET_X) + Option.OFFSET_X)){
	    ...
	    if(ClientMain.currentOption != null){
	    	if(x > GraphicsManager.CONFIRM_X && x < GraphicsManager.CONFIRM_X + GraphicsManager.CONFIRM_SIZE_X){
	    		if(y > GraphicsManager.CONFIRM_Y && y < GraphicsManager.CONFIRM_Y + GraphicsManager.CONFIRM_SIZE_Y){
	    			...
	    			for(int i = 0; i < ClientMain.currentOptions.options.size(); i++){
	    				if(!(ClientMain.currentOptions.options.get(i).text.equals(ClientMain.currentOption.text))){
